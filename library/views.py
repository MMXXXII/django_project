...
class UserProfileViewSet(GenericViewSet):
    ...
    def use_login(self, request):
        ...
        if user:
            ...
            cache.set(f'otp_pending_{user.username}', {'otp_code': otp_code,'timestamp': time.time(),'password': serializer.validated_data['password']}, 300)
    ...
    def get_user_info(self, request):
        return Response({'username': self.request.user.username,'email': self.request.user.email,'first_name': self.request.user.first_name,'last_name': self.request.user.last_name})
